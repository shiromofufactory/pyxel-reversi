...
const SCRIPT_DIR = import.meta.url.substring(
  0,
...
class Pyxel {
  ...
  package(appRootDir, startupScriptName) {
    this._pyodide.runPython(
      `import pyxel.cli; pyxel.cli.package_pyxel_app("${appRootDir}", "${startupScriptName}")`
    )
  }
...
function setCanvasHeight() {
  document.querySelector("canvas#canvas").style.height =
